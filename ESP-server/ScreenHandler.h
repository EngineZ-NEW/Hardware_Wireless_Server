...
int selectPage(){
  if(millis()-timeNow < 800){
    ...
    tft.setCursor(200, 150, 2);
    tft.printf("page: %i", page);
    tft.setCursor(5,200,2);
    tft.printf("Xdiff: %i", movedX-startX);
movedX=0;
        }
      }
  }
  ...
  void drawMenu (String headerText ){
    ...
    void drawLinesForRecors(){
      for(int i=0;i<6;i++)
         tft.fillRect(20, 80+(i*40), 440, 30,TFT_DARKDARKGREEN);
      }
    void drawSquares(){
          tft.fillRect(20, 90, 210,90,TFT_DARKDARKGREEN);
          tft.fillRect(250, 90,210,90,TFT_DARKDARKGREEN);
          tft.fillRect(20, 200, 210,90,TFT_DARKDARKGREEN);
          tft.fillRect(250, 200, 210,90,TFT_DARKDARKGREEN);
      }
...
int calculatePressQuadron(){
  return (((int)x/160)+1)*(((int)y/160)+1);
  }
